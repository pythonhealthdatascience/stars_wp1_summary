...
<!-- TODO: From Tom - So much great work and interesting findings. We need to move towards some headlines and take aways for the community. Mayybe there are some high level groupings of your findings? e.g.
...
* Evidence supporting the use of reporting guidelines
    * as in - reporting guidelines state this should info be included and you found it v.difficult or impossible to reproduce without it.?
-->
...
criteria_dict = {
  ...
  'Can run some programmatically': [7, 0, 1, 0],
  '''Don't hard code scenario param.''': [3, 2, 2, 1],
  'Relative file paths': [5, 0, 0, 3],
  'Avoid duplication': [5, 0, 3, 0],
  'Sufficient comments': [1, 3, 4, 0],
  'If slow, state run time': [2, 2, 3, 1],
  'Provide scenario code': [1, 1, 5, 1],
  'Provide all required outputs': [3, 0, 5, 0],
  'Include correct parameters': [4, 2, 2, 0],
  'Provide all required parameters': [4, 0, 4, 0],
  'If not, clearly state in paper': [1, 2, 2, 3],
  'If process param., provide calc.': [1, 0, 1, 6],
  'Save output to file': [4, 0, 4, 0],
  'Understandable output tables': [5, 0, 3, 0],
  'Use seeds': [5, 0, 3, 0],
  'Code to make tables': [1, 1, 4, 2],
  'Code to make figures': [1, 2, 5, 0],
  'Code to process in-text results': [0, 0, 3, 5],
  'Run instructions': [2, 1, 5, 0],
}
...
@johnson_cost_2021: Fully met. Model provided as a package (which is an R interface for the C++ model).
...
* This was not an issue for any studies - but included to note this was a "facilitator", as would have needed to amend if they weren't.
...
<!--TODO: Move this to other, or move things from other up here into their own boxes -->

## Do not output excessive numbers of files

I have not evaluated this as a criteria, as a large number of files is not inherently a bad thing, and might be difficult to avoid.
