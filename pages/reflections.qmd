...
<!--TODO: Check all the notes on Johnson's reflections page are captured below, and that there weren't any new categories to add-->
...
<!--TODO: Add stacked bar summary chart -->
...
criteria_dict = {
  'List packages': [2, 3, 3, 0],
  'Provide versions': [0, 6, 2, 0],
  'Model in "runnable" format': [7, 0, 1, 0],
  'Run programmatically': [0, 0, 0, 0],
  '''Don't hard code scenario param.''': [0, 0, 0, 0],
  'Relative file paths': [0, 0, 0, 0],
  'Avoid duplication': [0, 0, 0, 0],
  'Sufficient comments': [0, 0, 0, 0],
  'Quicker (?)': [0, 0, 0, 1],
  'If slow, state run time': [0, 0, 0, 0],
  'If comp. exp., state usage + alt.': [0, 0, 0, 0],
  'Provide scenario code': [0, 0, 0, 0],
  'Provide all required outputs': [0, 0, 0, 0],
  'Include correct parameters': [0, 0, 0, 0],
  'Provide all required parameters': [0, 0, 0, 0],
  'If not, clearly state in paper': [0, 0, 0, 0],
  'If process param., provide calc.': [0, 0, 0, 0],
  'Save output to file': [0, 0, 0, 0],
  'Understandable output tables': [0, 0, 0, 0],
  'Avoid large files (?)': [0, 0, 0, 1],
  'Not excessive output files': [0, 0, 0, 0],
  'Use seeds': [0, 0, 0, 0],
  'Code to make tables': [0, 0, 0, 0],
  'Code to make figures': [0, 0, 0, 0],
  'Code to process in-text results': [0, 0, 0, 0],
  'Run instructions': [0, 0, 0, 0],
}
...
@johnson_cost_2021: Not applicable. All inputs defined within script. Outputs are not specifically saved to a file (just that the .md and image files were automatically saved when the .Rmd file was knit).
...
<!--TODO: Consider whether this should be done like a checklist like this? As actually, I could run a simple version (e.g. serial or one at a time) without issue, but starts taking up more memory with parallel stuff.-->

| @shoaib_simulation_2021 | @huang_optimizing_2019 | @lim_staff_2020 | @kim_modelling_2021 | @anagnostou_facs-charm_2022 | @johnson_cost_2021 | @hernandez_optimal_2015 | @wood_value_2021 |
| :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: |
| N/A | N/A | N/A | ❌ | N/A | ❌ | <!--TODO--> | N/A |
...
@johnson_cost_2021: Not met. It becomes more computationally expensive if try to run lots at once in simultaneous terminals. Didn't try running one on local machine with full parameter due to long run time making it infeasible, but knowing my system specs, it should have been able to if did.
...
@johnson_cost_2021: Fully met. Could determine appropriate parameters for sensitivity analysis from description in article.
...
* @johnson_cost_2021: Not relevant.
